...
async def start(message: types.Message):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			...
			await message.answer('Приветствую, это бот для общения с пользователем.\nНапиши мне свой вопрос и я отправлю его пользователю.\nЗа спам/флуд - ЧС!')
	else:
...
async def proc(message: types.Message, state: FSMContext):
	if message.text == '⏪ Отмена':
		...
		id = data.get("uid")
		await state.finish()
		await bot.send_message(id, 'Вам поступил ответ от пользователя:\n\nТекст: {}'.format(message.text))
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 0:
					...
					await state.finish()
					await bot.send_message(message.text, 'Ты заблокирован.')
				else:
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 1:
					...
					await state.finish()
					await bot.send_message(message.text, 'Вы были разблокированы.')
				else:
